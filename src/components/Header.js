      ...
      <div className=" h-full container mx-auto flex items-center px-4 justify-between">
        ...
        <div className="flex items-center gap-7">
          <div className="relative flex justify-center">
            ...
            {menuDisplay && (
              <div
                className="absolute hover:bg-slate-100 bg-white bottom-0 top-11 h-fit p-2 shadow-lg rounded "
                onClick={() => setmenuDisplay((preve) => !preve)}
              >
                <nav>
                  {user?.role === ROLE.ADMIN && (
                    <Link
                      ...
                      className="whitespace-nowrap p-2 hidden md:block"
                    >
                      Admin Panel
                    </Link>
                  )}
          ...
          <div>
            {user?._id ? (
              ...
              <Link
                to={"/login"}
                className="px-4 py-1 rounded-full text-white flex items-center bg-red-600 hover:bg-red-700"
              >
                Login
              </Link>
            )}
